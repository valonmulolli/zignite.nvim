...
var c_family_auto_cache: std.StringHashMap(CacheEntry) = .init(page_allocator);

pub fn getCFamilyAutoOutput(options: Options, signature: []const u8) !?[]const u8 {
    const cache_key = try cacheKeyAlloc(page_allocator, options);
    defer page_allocator.free(cache_key);

    const entry = c_family_auto_cache.get(cache_key) orelse return null;
    if (!std.mem.eql(u8, entry.signature, signature)) return null;
    return entry.output;
}

pub fn storeCFamilyAutoOutput(options: Options, signature: []const u8, output: []const u8) !void {
    const cache_key = try cacheKeyAlloc(page_allocator, options);
    ...
    const gop = try c_family_auto_cache.getOrPut(cache_key);
...
pub fn resetForTests() void {
    var iterator = c_family_auto_cache.iterator();
    ...
    c_family_auto_cache.clearRetainingCapacity();
}
